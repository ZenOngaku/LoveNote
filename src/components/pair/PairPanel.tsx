...
export function PairPanel({
  ...
  return (
    <Tabs value={mode} onValueChange={setMode}>
      {/* 模式切换 */}
      <TabsList className="mb-4 grid h-11 w-full grid-cols-2 rounded-full bg-rose-100/70 p-1">
        <TabsTrigger
          value="generate"
          className="rounded-full text-sm data-[state=active]:bg-white data-[state=active]:text-rose-500 data-[state=active]:shadow-sm"
        >
          生成邀请码
        </TabsTrigger>
        <TabsTrigger
      ...
      <TabsContent value="generate">
        {pendingInvite ? (
          ...
          <div className="rounded-3xl border border-rose-100 bg-white p-8 text-center shadow-sm">
            ...
            <h3 className="mt-4 font-semibold text-stone-800">生成专属邀请码</h3>
      ...
      <TabsContent value="enter">
        <div className="rounded-3xl border border-rose-100 bg-white p-8 shadow-sm">
          <div className="text-center">
            <h3 className="font-semibold text-stone-800">输入 TA 给你的邀请码</h3>
            <p className="mt-2 text-sm text-stone-400">6 位数字，输入后即可完成情侣绑定</p>
          </div>
              ...
              <InputOTPGroup className="gap-2">
                {[0, 1, 2, 3, 4, 5].map((i) => (
                  <InputOTPSlot
                    key={i}
                    index={i}
                    className="h-14 w-11 rounded-xl border-rose-200 bg-rose-50/50 text-xl font-bold text-rose-500 first:rounded-xl last:rounded-xl data-[active=true]:border-rose-400"
                  />
                ))}
              </InputOTPGroup>
            </InputOTP>
...
function InviteCodeDisplay({
  ...
  return (
    <div className="rounded-3xl border border-rose-100 bg-white p-8 text-center shadow-sm">
      <p className="text-sm text-stone-400">你的专属邀请码</p>

      {/* 大号邀请码展示 */}
      <p
        className="mt-4 select-all text-4xl font-bold tracking-[0.3em] text-rose-500"
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <Button
          ...
          disabled={acting}
          onClick={onCopy}
          className="h-11 rounded-full border-rose-200 text-rose-500 hover:bg-rose-50 hover:text-rose-600"
        >
      ...
      <Button
        ...
        disabled={acting}
        onClick={onCancel}
        className="mt-2 h-10 w-full rounded-full text-sm text-stone-400 hover:bg-stone-50 hover:text-stone-500"
      >
        取消邀请
      </Button>
    </div>
